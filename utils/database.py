...
# Replaces the monolithic .bc_cache.json with structured, indexed tables.
...
def initialize_database(db_path: Path = DB_PATH):
    ...
    # ------------------------------------------------------------------
    # 8. Roster cache — avoids re-scraping Liquipedia team pages
    # ------------------------------------------------------------------
    c.execute("""
...
def cache_api_response(cache_key: str, endpoint: str, raw_json: str,
    ...
    """Insert or update a generic API response in the cache."""
    own = conn is None
    if own:
        conn = get_connection()
...
def get_cached_api_response(cache_key: str,
    ...
    """Return the parsed JSON for a cached API response, or None."""
    own = conn is None
    if own:
        conn = get_connection()
    row = conn.execute(
...
def cache_replay(replay_id: str, date: str, playlist_id: str,
                 ...
                 conn: Optional[sqlite3.Connection] = None):
    """Insert or update a replay in the dedicated replay table."""
    own = conn is None
    if own:
        conn = get_connection()
    conn.execute(
...
def get_cached_replay(replay_id: str,
                      conn: Optional[sqlite3.Connection] = None) -> Optional[dict]:
    """Return the parsed JSON for a cached replay, or None."""
    own = conn is None
    if own:
        conn = get_connection()
    row = conn.execute(
...
def get_all_replay_details(conn: Optional[sqlite3.Connection] = None) -> list[dict]:
    """Return ALL cached replay detail dicts. Used by features.py / train.py."""
    own = conn is None
    if own:
        conn = get_connection()
...
def cache_player_stats(replay_id: str, player_name: str, side: str,
                       ...
                       conn: Optional[sqlite3.Connection] = None):
    """Insert a single player-stat row."""
    own = conn is None
    if own:
        conn = get_connection()
    conn.execute("""
...
def load_player_id_map_db(conn: Optional[sqlite3.Connection] = None) -> dict:
    """Reconstruct the same {aliases: {}, players: {}} dict from the DB."""
    own = conn is None
    if own:
        conn = get_connection()
...
def import_ids_json(json_path: str, conn: Optional[sqlite3.Connection] = None):
    """One-time migration: load an ids.json file into the DB tables."""
...
def migrate_json_cache(cache_path: str = ".bc_cache.json",
                       conn: Optional[sqlite3.Connection] = None) -> int:
    """
    Migrate the existing .bc_cache.json into the SQLite database.

    - Every key/value goes into api_cache (preserving the MD5 keys)
    - Replay details are additionally extracted into the replays table
    - Player stats are extracted into player_stats
    ...
    for key, data in cache.items():
        ...
        # Store in general api_cache
        conn.execute(
            "INSERT OR IGNORE INTO api_cache (cache_key, endpoint, raw_json, cached_at) VALUES (?, ?, ?, ?)",
            (key, "", raw, now),
        )

        # If this looks like a replay detail, also store in the replays table
        ...
        count += 1
        if count % 1000 == 0:
            conn.commit()  # periodic commit to avoid holding too much in memory
...
def save_tournament(url: str, name: Optional[str] = None,
                    conn: Optional[sqlite3.Connection] = None) -> int:
    """Insert a tournament and return its ID (or the existing one)."""
    own = conn is None
    if own:
        conn = get_connection()
...
def save_matchups(tournament_id: int, rows: list[dict],
                  conn: Optional[sqlite3.Connection] = None):
    """Bulk-insert matchup rows for a tournament."""
    own = conn is None
    if own:
        conn = get_connection()
...
def cache_roster(team_url: str, players: list[str],
                 conn: Optional[sqlite3.Connection] = None):
    """Cache a team's roster."""
    own = conn is None
    if own:
        conn = get_connection()
    now = datetime.now(timezone.utc).isoformat()
...
def get_cached_roster(team_url: str,
                      conn: Optional[sqlite3.Connection] = None) -> Optional[list[str]]:
    """Return cached roster list or None."""
    own = conn is None
    if own:
        conn = get_connection()
    row = conn.execute(
...
def export_database_file(destination_path: str) -> bool:
    """Copies the current SQLite database to a destination (e.g., for user download)."""
    if DB_PATH.exists():
...
def import_database_file(source_path: str) -> bool:
    """Replaces the current SQLite database with an uploaded one."""
