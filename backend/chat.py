...
File Function:
- Description: Interactive terminal chatbot for querying Over/Under predictions
- Usage: Loaded by main.py in chat mode, allows interactive queries
"""
...
from scrapers.h2h_ballchasing import Ballchasing, get_h2h_stats, load_player_id_map, resolve_ids
...
def log_prediction(player, stat, threshold, is_over, num_games, nn_prob, features, team1="", team2=""):
    """
    Description:
        Logs an O/U prediction query to a persistent CSV log.
    Arguments:
        player: Player name.
        stat: Statistic name.
        threshold: Prediction threshold.
        is_over: True if Over, False if Under.
        num_games: Number of games predicted.
        nn_prob: NN output probability.
        features: 13-dimensional feature vector.
        team1: Team 1 name.
        team2: Team 2 name.
    Returns:
        None
...
def parse_query(query: str, available_players=None):
    """
    Description:
        Parses an interactive natural language prediction query.
    Arguments:
        query: Raw query text.
        available_players: Set of player names active in current match.
    Returns:
        Tuple: (player, stat, is_over, threshold, num_games)
    """
    query_lower = query.lower()

    is_over = None
    if "over" in query_lower or "o/" in query_lower: is_over = True
    elif "under" in query_lower or "u/" in query_lower: is_over = False
    ...
    if stat: known.add(stat.lower())
    for num_str, _ in all_numbers: known.add(num_str)
...
def run_chat(row, bc, id_map):
    """
    Description:
        Executes the main terminal O/U predictor chatbot loop.
    Arguments:
        row: Selected match row series from playoff scraper.
        bc: Ballchasing client.
        id_map: Roster player ID mapping dictionary.
    Returns:
        None
    """
    ...
    if r1 and r2: print(f"Rosters: {', '.join(r1)} vs {', '.join(r2)}")

    nn_model = load_model()
    if nn_model: print("Neural Net model loaded!")
    else: print("No trained model found. Run 'python3 train.py' to train.")
    ...
    gen_df = replay_stats(bc, all_roster_ids, logs)

    print("[Step 2/3] Fetching ranked 2s momentum...")
    ranked_momentum = ranked_activity(bc, all_roster_ids, logs)

    print("[Step 3/3] Fetching head-to-head history...")
    h2h_df, h2h_logs = get_h2h_stats(t1, t2, r1, r2, bc)
    print("Done!\n")
    ...
    if not h2h_df.empty:
        ...
        if len(expected_players) > 0 and len(overlap) >= 4: confident_h2h = True

        if confident_h2h: print("High Confidence: H2H data heavily matches the current rosters!")
        else: print("Low Confidence: H2H history features mostly old or different rosters.")
    ...
    for p in canonical_players:
        ...
        if cid not in present_canonical_ids: continue
    ...
    while True:
        ...
        if norm_q in alias_lookup: matched_player_dict = alias_lookup[norm_q]
        else:
        ...
        players_table = id_map.get("players", {})
        ...
        if nn_model:
            ...
            if best_prob is not None: display_prob = best_prob / 100.0
