...
File Function:
- Description: Trains the RLPredictor neural network MLP model on self-supervised DB replays and live logs.
- Usage: Executed via command-line (python3 train.py) to save weights to predictor/backend/data/model.pt.
"""
...
from model import train_model, save_model, predict, HAS_TORCH


def main():
    """
    Description:
        Main CLI parsing and training execution entry point.
    Arguments:
        None
    Returns:
        None
    ...
    if not HAS_TORCH: print("PyTorch is not installed. Run: pip install torch"); sys.exit(1)
    ...
    if os.path.exists(log_path):
        try:
            ...
            if not df_live.empty:
                ...
                print(f"[OK] Total samples now: {len(features)} ({len(df_live)} from live log)")
        except Exception as e:
            print(f"[WARNING] Could not load live log: {e}")

    if len(features) == 0:
        print("No training data could be generated. Make sure predictor.db has replay data.")
